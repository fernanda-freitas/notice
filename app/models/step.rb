class Step < ApplicationRecord
  ...
  mattr_accessor :form_multisteps do
  ...
  def required_for_multistep?(step)
    # All fields are required if no form multistep is present
    return true if form_step.nil?

    # All fields from previous steps are required if the
    # step parameter appears before or we are on the current step
    return true if self.form_multisteps.index(step.to_s) <= self.form_multisteps.index(form_step)
  end
end
